const axios = require('axios');
const { SlashCommandBuilder } = require('discord.js');

/**
 * Function calls the API endpoint to validate the country name with their database list of countries
 *      -> //https://api.covid19api.com/total/country/india/status/confirmed?from=2020-03-01&to=2020-03-05
 * @param {string} countryName name of country provided by user in discord
 * @returns true if the cuontry is in the database
 */
async function verifyCountry(countryName) {
    try {
        const response = await axios.get('https://api.covid19api.com/countries');
        var resStr = await JSON.stringify(response.data);
        var resJSON = await JSON.parse(resStr);
        let countryValid = false;

        let lowerCountry;
        for (let i = 0; i < resJSON.length; i++) {
            lowerCountry = resJSON[i].Country.toLowerCase();
            if (lowerCountry == countryName) {
                countryValid = true;
            }
        }

        return countryValid;
    } catch (error) {
        console.error(error);
    }
}

/**
 * Calls endpoint with dates and country name and returns msg with details 
 * @param {*} country 
 * @param {*} dd1, mm1, yyy1  - Date from
 * @param {*} dd2, mm2, yyy2  - Date until 
 * @returns Covid cases from date1 to date2
 */
async function covidRecordsOnDate(country, dd1, mm1, yy1, dd2, mm2, yy2) {
    try {

        if (await verifyCountry(country)) {

            //reformating dates if they are not two digited
            if (dd1 < 10) {
                dd1 = "0" + dd1;
            }
            if (dd2 < 10) {
                dd2 = "0" + dd2;
            }
            if (mm1 < 10) {
                mm1 = "0" + mm1;
            }
            if (mm2 < 10) {
                mm2 = "0" + mm2;
            }
            let fromDate = yy1 + "-" + mm1 + "-" + dd1;
            let untilDate = yy2 + "-" + mm2 + "-" + dd2;

            const response2 = await axios.get('https://api.covid19api.com/total/country/' + country + '/status/confirmed?from=' + fromDate + '&to=' + untilDate);

            var resStr = await JSON.stringify(response2.data);
            var resJSON = await JSON.parse(resStr);

            //main msg
            let msg = "Total cases for " + country + " from: " + fromDate + " to:" + untilDate + " were ";

            //msg with details on each date (list)
            let detailedMsg = "";

            //storing converted DateTime into readable format by using slicing 
            let shortDate;

            //covid case count
            let countCases = 0;

            //discord msg: Must be 2000 or fewer in length. 
            //   -> If user provides two dates with big difference, triming returning records.
            let reducedLength = resJSON.length;
            if (reducedLength > 20) {
                reducedLength = 20;
                detailedMsg = "\nPlease use nearer dates. Can not show more than 20 results."
            }


            for (let i = 0; i < reducedLength; i++) {
                countCases += resJSON[i].Cases;
                shortDate = resJSON[i].Date.slice(0, resJSON[i].Date.length - 10);
                detailedMsg += "\nOn " + shortDate + " cases were " + resJSON[i].Cases + ".";
            }
            msg += countCases + ".";
            msg = msg + detailedMsg;

            return msg;
        } else {
            return "Oops! Sorry not able to find any record for " + country;
        }
    } catch (error) {
        console.error(error);
    }

}

/**
 *  /casesbetweendates [country] [1_dd][1_mm][1_yyyy] [2_dd][2_mm][2_yyyy]
 *                  -> command returns covid cases for country in-between provided dates
 *                  -> unable to find other best way to get validated dates as user-input
 *          //https://api.covid19api.com/total/country/india/status/confirmed?from=2020-03-01&to=2020-03-05
 */
 module.exports = {
    data: new SlashCommandBuilder()
        .setName('casesbetweendates')
        .setDescription('Total number of covid cases for country between provided dates')
        .addStringOption(option => option.setName('country')              //string input - search term for gif
            .setDescription('Country name to get all covid cases')
            .setRequired(true))
        .addNumberOption(option => option.setName('1_dd')
            .setMinValue(1)
            .setMaxValue(31)
            .setRequired(true)
            .setDescription('From Day'))
        .addNumberOption(option => option.setName('1_mm')
            .setMinValue(1)
            .setMaxValue(12)
            .setRequired(true)
            .setDescription('From Month'))
        .addNumberOption(option => option.setName('1_yyyy')
            .setMinValue(2020)
            .setMaxValue(2022)
            .setRequired(true)
            .setDescription('From year'))
        .addNumberOption(option => option.setName('2_dd')
            .setMinValue(1)
            .setMaxValue(31)
            .setRequired(true)
            .setDescription('Until Day'))
        .addNumberOption(option => option.setName('2_mm')
            .setMinValue(1)
            .setMaxValue(12)
            .setRequired(true)
            .setDescription('Until Month'))
        .addNumberOption(option => option.setName('2_yyyy')
            .setMinValue(2020)
            .setMaxValue(2022)
            .setRequired(true)
            .setDescription('Until year'))

    ,

    async execute(interaction) {
        const country = interaction.options.getString('country');
        const dd1 = interaction.options.getNumber("1_dd");
        const mm1 = interaction.options.getNumber("1_mm");
        const yyyy1 = interaction.options.getNumber("1_yyyy");
        const dd2 = interaction.options.getNumber("2_dd");
        const mm2 = interaction.options.getNumber("2_mm");
        const yyyy2 = interaction.options.getNumber("2_yyyy");

        return interaction.reply(await covidRecordsOnDate(country, dd1, mm1, yyyy1, dd2, mm2, yyyy2));
    }

}